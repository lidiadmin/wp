...
final class Component_Registry {
	...
	public static function add( array $data ) {
		...
		if ( empty( $data['type'] ) || ! in_array( $data['type'], self::$supported_component_types, true ) ) {
			throw new InvalidArgumentException( esc_html__( '必须提供有效的组件类型', 'dahuzi-blocks' ) );
		}

		if ( empty( $data ) ) {
			throw new InvalidArgumentException( __( '必须提供有效的布局数据才能注册布局', 'dahuzi-blocks' ) );
		}
		...
		foreach ( self::$required_data_keys as $required_key ) {
			if ( ! array_key_exists( $required_key, $data ) || empty( $data[ $required_key ] ) ) {
				...
				throw new InvalidArgumentException( sprintf( esc_html__( '必须提供 %s 才能注册布局', 'dahuzi-blocks' ), $required_key ) );
			}
		}
		...
		switch ( $data['type'] ) {
			case 'layout':
				if ( ! empty( self::$layouts[ $data['key'] ] ) ) {
					...
					throw new InvalidArgumentException( sprintf( esc_html__( '%s 布局已注册', 'dahuzi-blocks' ), $data['key'] ) );
				}
			...
			case 'section':
				if ( ! empty( self::$sections[ $data['key'] ] ) ) {
					/* translators: %s: The component's unique key. */
					throw new InvalidArgumentException( sprintf( esc_html__( '%s 分区已注册', 'dahuzi-blocks' ), $data['key'] ) );
				}
				self::$sections[ $data['key'] ] = $data;
				break;

			default:
				/* translators: %s: This functions name. Will always be DahuziBlocks\Layouts\Component_Registry::add(). */
				throw new InvalidArgumentException( sprintf( esc_html__( '必须在 %s 中提供有效的组件类型', 'dahuzi-blocks' ), __METHOD__ ) );
		}
	}
	...
	public static function remove( $type, $key ) {
		...
		if ( empty( $type ) || ! in_array( $type, self::$supported_component_types, true ) ) {
			/* translators: %s: This functions name. Will always be DahuziBlocks\Layouts\Component_Registry::remove(). */
			throw new InvalidArgumentException( sprintf( esc_html__( '必须在 %s 中提供有效的组件类型', 'dahuzi-blocks' ), __METHOD__ ) );
		}

		if ( empty( $key ) ) {
			/* translators: %s: This functions name. Will always be DahuziBlocks\Layouts\Component_Registry::remove(). */
			throw new InvalidArgumentException( sprintf( esc_html__( '必须在 %s 中提供有效的组件密钥', 'dahuzi-blocks' ), __METHOD__ ) );
		}
		...
		switch ( $type ) {
			case 'layout':
				if ( empty( self::$layouts[ $key ] ) ) {
					...
					throw new Exception( sprintf( esc_html__( '%s 布局未注册', 'dahuzi-blocks' ), $key ) );
				}
			...
			case 'section':
				if ( empty( self::$sections[ $key ] ) ) {
					/* translators: The requested components unique key. */
					throw new Exception( sprintf( esc_html__( '%s 节点未注册', 'dahuzi-blocks' ), $key ) );
				}
	...
	public static function get( $type, $key ) {
		...
		if ( empty( $type ) || ! in_array( $type, self::$supported_component_types, true ) ) {
			/* translators: This function name. Will always be DahuziBlocks\Layouts\Component_Registry::get(). */
			throw new Exception( sprintf( esc_html__( '必须在 %s 中提供组件类型', 'dahuzi-blocks' ), __METHOD__ ) );
		}
