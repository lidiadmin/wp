 ...
 * 布局块相关功能
 *
 * @package DahuziBlocks
 */

use DahuziBlocks\Layouts\Component_Registry;
 ...
 */
function dahuzi_blocks_register_layout_component( array $data ) {
 ...
 */
function dahuzi_blocks_unregister_layout_component( $type, $key ) {
	$registry = Component_Registry::instance();
	try {
 ...
 */
function dahuzi_blocks_get_layout_component( $type, $key ) {

	if ( empty( $type ) ) {
		return new WP_Error( esc_html__( '必须提供类型才能检索布局组件', 'dahuzi-blocks' ) );
	}

	if ( empty( $key ) ) {
		return new WP_Error( esc_html__( '必须提供键才能检索布局组件', 'dahuzi-blocks' ) );
	}
 ...
 */
function dahuzi_blocks_get_layouts() {
	$registry = Component_Registry::instance();
 ...
 */
function dahuzi_blocks_get_sections() {
	$registry = Component_Registry::instance();
