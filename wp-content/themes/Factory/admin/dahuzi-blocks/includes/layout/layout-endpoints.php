...
const DB_API_NAMESPACE = 'dahuziblocks/v1';
...
function register_layout_endpoints() {
	...
	register_rest_route(
		...
		[
			...
			'callback'            => function () {
				return new WP_REST_Response( (array) get_user_meta( get_current_user_id(), 'dahuzi_blocks_favorite_layouts', true ) );
			},
	...
	register_rest_route(
		...
		[
			...
			'callback'            => function ( \WP_REST_Request $request ) {
				...
				$layouts            = dahuzi_blocks_get_layouts();
				$sections           = dahuzi_blocks_get_sections();
				$additional_layouts = apply_filters( 'dahuzi_blocks_additional_layout_components', [] );
				 ...
				 *
				 */
				$allowed_layouts = (array) apply_filters( 'dahuzi_blocks_allowed_layout_components', array_keys( $all_layouts ) );
	...
	register_rest_route(
		...
		[
			'methods'             => WP_REST_Server::READABLE,
			'callback'            => function () {
				return new WP_REST_Response( (array) dahuzi_blocks_get_layouts() );
			},
			'permission_callback' => function () {
				return current_user_can( 'edit_posts' );
			},
	...
	register_rest_route(
		...
		[
			...
			'callback'            => function ( $request ) {
				...
				return new WP_REST_Response( esc_html__( '找不到布局', 'dahuzi-blocks' ) );
			},
			'permission_callback' => function () {
				return current_user_can( 'edit_posts' );
			},
	...
	register_rest_route(
		...
		[
			...
			'callback'            => function ( $request ) {
				...
				$new       = sanitize_key( $body['dahuzi_blocks_favorite_key'] );
				$favorites = (array) get_user_meta( get_current_user_id(), 'dahuzi_blocks_favorite_layouts', true );
				...
				update_user_meta( get_current_user_id(), 'dahuzi_blocks_favorite_layouts', array_values( $favorites ) );
	...
	register_rest_route(
		...
		[
			...
			'callback'            => function ( $request ) {
				...
				$body      = json_decode( $request->get_body(), true );
				$delete_id = sanitize_key( $body['dahuzi_blocks_favorite_key'] );
				$favorites = (array) get_user_meta( get_current_user_id(), 'dahuzi_blocks_favorite_layouts', true );
	...
	register_rest_route(
		...
		[
			'methods'             => WP_REST_Server::READABLE,
			'callback'            => function () {
				return new WP_REST_Response( (array) dahuzi_blocks_get_sections() );
			},
			'permission_callback' => function () {
				return current_user_can( 'edit_posts' );
			},
