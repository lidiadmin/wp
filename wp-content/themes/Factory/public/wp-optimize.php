 ...
 *  --------------------------------------------------------------------------------- */
if ( dahuzi('dahuzi_tag_rewrite') ) {
	add_action('generate_rewrite_rules','dahuzi_tag_rewrite_rules');
	add_filter('term_link','dahuzi_tag_term_link',10,3);
	add_action('query_vars', 'dahuzi_tag_query_vars');
}

function dahuzi_tag_rewrite_rules($wp_rewrite){
	$new_rules = array(
...
function dahuzi_tag_term_link($link,$term,$taxonomy){
...
function dahuzi_tag_query_vars($public_query_vars){
 ...
 *  给分类目录链接末尾添加 / 斜杠
 *  --------------------------------------------------------------------------------- */
if( dahuzi('dahuzi_trailingslashit') ) :
 ...
 *  在后台添加一些css样式
 *  --------------------------------------------------------------------------------- */
add_action( 'admin_head', 'dahuzi_admin_head' );
function dahuzi_admin_head(){

	echo "<style>
	.manage-column.column-views,.views.column-views{width:7%}
	.dahuzi_img.column-dahuzi_img,.manage-column.column-dahuzi_img{width:100px}
	</style>";
	if( dahuzi('remove_menu_xintheme-license') ){
		echo "<style>.toplevel_page_xintheme-page .wp-first-item+li{display:none}</style>";
	}
	if( dahuzi('remove_menu_xintheme-optimize') ){
		echo "<style>.toplevel_page_xintheme-page .wp-first-item+li+li{display:none}</style>";
	}
 ...
 *  彻底移除后台「评论」相关菜单
 *  --------------------------------------------------------------------------------- */
if( dahuzi('dahuzi_remove_comment') ) :
 ...
 *  移除后台「仪表盘」菜单
 *  --------------------------------------------------------------------------------- */
if( dahuzi('dahuzi_remove_dashboard') ) :
 ...
 *  修改网站管理员邮箱 去除验证步骤
 *  --------------------------------------------------------------------------------- */
if( dahuzi('dahuzi_new_admin_email') ) :
...
if( !defined('WPJAM_BASIC_PLUGIN_FILE') ){
	...
	//
	//在后台文章列表增加一列数据
	add_filter( 'manage_post_posts_columns', 'xintheme_customer_posts_columns' );
	...
	//添加对列进行排序的功能
	add_filter('manage_edit-post_sortable_columns', 'dahuzi_add_views_sortable_column');
	function dahuzi_add_views_sortable_column($columns){
		$columns['views'] = '浏览量';
		return $columns;
	}

	//输出浏览次数
	add_action('manage_post_posts_custom_column', 'xintheme_customer_columns_value', 10, 2);
	...
	// 后台文章列表增加「缩略图」显示
	//
	if( dahuzi('dahuzi_admin_column_img') ){ //判断是否开启文章缩略图

		/**
		* manage_post_posts_columns 为默认文章类型添加columns
		* manage_dahuzi_posts_columns 为自定义文章类型（dahuzi）添加columns
		*/
		add_filter('manage_post_posts_columns', 'add_img_column_dahuzi');
		add_filter('manage_post_posts_custom_column', 'manage_img_column_dahuzi', 10, 2);

		function add_img_column_dahuzi($columns) {
			$columns = array_slice($columns, 0, 1, true) + array("dahuzi_img" => "缩略图") + array_slice($columns, 1, count($columns) - 1, true);
			return $columns;
		}

		function manage_img_column_dahuzi($column_name, $post_id) {
			if( $column_name == 'dahuzi_img' ) {
	...
	// 后台用户列表增加「最后登录时间」显示
	//
	add_action( 'wp_login', 'dahuzi_collect_login_timestamp', 20, 2 );
	function dahuzi_collect_login_timestamp( $user_login, $user ) {
		update_user_meta( $user->ID, 'last_login', time() );
	}

	add_filter( 'manage_users_columns', 'dahuzi_user_last_login_column' );
	function dahuzi_user_last_login_column( $columns ) {
		$columns['last_login'] = '最后登录'; //
		return $columns;
	}

	add_filter( 'manage_users_custom_column', 'dahuzi_last_login_column', 10, 3 );
	function dahuzi_last_login_column( $output, $column_id, $user_id ){
		if( $column_id == 'last_login' ) {
			$last_login = get_user_meta( $user_id, 'last_login', true );
			$output = $last_login ? get_date_from_gmt(date('Y-m-d H:i:s', $last_login)) : '';
		}
		return $output;
	}

	//最后登录时间 增加可排序功能
	add_filter( 'manage_users_sortable_columns', 'dahuzi_sortable_columns' );
	function dahuzi_sortable_columns( $columns ) {
	...
	add_action( 'pre_get_users', 'dahuzi_sort_last_login_column' );
	function dahuzi_sort_last_login_column( $query ) {
	...
	// 后台-用户，可通过用户昵称进行搜索
	//
	add_action('pre_user_query', 'dahuzi_extend_user_search');
	function dahuzi_extend_user_search( $u_query ){
	    // 确保代码仅应用于用户搜索
