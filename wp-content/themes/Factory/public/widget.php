 ...
 * @Last Modified time: 2021-05-18 16:35:17
 */
...
if( ! function_exists( 'widget_footer_1' ) ) {
  function widget_footer_1( $args, $instance ) {
	...
	<?php if( $instance['widget_footer1_logo']['url'] ){?>
		<img class="footer-logo mb-25" src="<?php echo $instance['widget_footer1_logo']['url'];?>" alt="<?php echo get_post_meta( $instance['widget_footer1_logo']['id'], '_wp_attachment_image_alt', true );?>">
	<?php }else{?>
	...
	<ul class="social-icon bg-transparent bordered-theme">
		<?php if( $instance['social_qq'] ){?>
		<li>
		<?php if( wp_is_mobile() ){?>
			<a href="mqqwpa://im/chat?chat_type=wpa&uin=<?php echo $instance['social_qq']; ?>&version=1&src_type=web&web_src=oicqzone.com" title="QQ" target="_blank" rel="nofollow"><i class="fa fa-qq"></i></a>
		<?php }else{?>
			<a href="http://wpa.qq.com/msgrd?v=3&uin=<?php echo $instance['social_qq']; ?>&site=qq&menu=yes" title="QQ" target="_blank" rel="nofollow"><i class="fa fa-qq"></i></a>
		<?php }?>
		</li>
		<?php }?>
		<?php if( $instance['social_weixin'] ){?>
		<li>
	    <a data-fancybox href="#social_weixin"><i class="fa fa-weixin"></i></a>
		</li>
		<div id="social_weixin" style="display:none;">
			<img src="<?php echo $instance['social_weixin']['url'];?>">
			<p style="text-align:center;font-size:16px;color:#333">微信扫一扫</p>
		</div>
		<?php }?>
		<?php if( $instance['social_weibo'] ){?>
		<li>
	    <a href="<?php echo $instance['social_weibo']; ?>" target="_blank" rel="nofollow"><i class="fa fa-weibo"></i></a>
		</li>
		<?php }?>
		<?php if( $instance['social_mail'] ){?>
		<li>
	    <a href="mailto:<?php echo $instance['social_mail']; ?>"><i class="fa fa-envelope"></i></a>
		</li>
		<?php }?>
	</ul>
	<?php }?>
	...
	<ul class="footer-list">
		<?php
			...
			<div class="blog-small-item">
			  <img src="<?php echo post_thumbnail(68, 68); ?>" alt="<?php echo post_thumbnail_alt();?>" />
			  <div class="tex">
...
if( ! function_exists( 'widget_footer_4' ) ) {
  function widget_footer_4( $args, $instance ) {
	...
	<ul class="footer-list">
		<img src="<?php echo $instance['widget_footer4_img']['url'];?>" alt="<?php echo get_post_meta( $instance['widget_footer4_img']['id'], '_wp_attachment_image_alt', true );?>">
	</ul>
...
class xintheme_postlist_img extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct('xintheme_postlist_img', __('文章展示（小图）'), $widget_ops);
	}
...
function xintheme_widget_postlist_img($orderby,$limit,$cat){
	...
	<div class="blog-small-item">
		<a href="<?php the_permalink(); ?>">
			<img src="<?php echo post_thumbnail(120, 120); ?>" alt="<?php echo post_thumbnail_alt();?>" />
		</a>
		<div class="tex">
		<h5><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h5>
		</div>
...
class xintheme_postlist extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array( 'description' => '可以选择显示最新文章、随机文章。' );
		parent::__construct('xintheme_postlist', __('文章展示（无图）'), $widget_ops);
	}
...
class xintheme_postlist_related extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct('xintheme_postlist_related', __('相关推荐（小图）'), $widget_ops);
	}
