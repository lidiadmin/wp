...
echo ' ';
    ...
    if($paged != 1){echo "<li><a href='" . get_pagenum_link(1) . "' class='extend' title='跳转到首页'> <i class='fa fa-angle-double-left'></i> </a></li>";}
    ...
    if($paged != $max_page){echo "<li><a href='" . get_pagenum_link($max_page) . "' class='extend' title='跳转到最后一页'> <i class='fa fa-angle-double-right'></i> </a></li>";}}  
}
...
function get_breadcrumbs()  {
    ...
    if ( !is_home() ){
        ...
        // Add the Home link  
        echo '<i class="fa fa-home"></i> <a href="'. get_option('home') .'">首页</a>';
        ...
        elseif ( is_tag() )  {
            echo " <span>&raquo;</span> 标签 <span>&raquo;</span> ".single_cat_title($prefix,$display)."";
        }
        elseif ( is_author() )  {
            echo " <span>&raquo;</span> 作者页面";
        }
        elseif ( is_archive() && !is_category() )  {
            echo " <span>&raquo;</span> 归档";
        }
        elseif ( is_search() ) {
            echo ' <span>&raquo;</span> “'.get_search_query().'” 的搜索结果（共搜索到 ' . $wp_query->found_posts . ' 篇文章）';
        }
        elseif ( is_404() )  {
        ...
        elseif ( is_single() )  {
            ...
            if($category){
                ...
                if( xintheme('breadcrumbs_post_title') ){
                    echo '正文';
                }else{
                    echo get_the_title();
                }
...
// 页面支持摘要
function dahuzi_excerpt_page() {
    add_post_type_support('page', array('excerpt'));
}
add_action('init', 'dahuzi_excerpt_page');
