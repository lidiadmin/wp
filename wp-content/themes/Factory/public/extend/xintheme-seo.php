 ...
 * @Last Modified time: 2021-04-10 19:34:23
 */
if( dahuzi('dahuzi_seo_switch') ){ //判断是否开启主题SEO设置
...
function dahuzi_seo_title() {
    ...
    if( is_category() ){
        ...
        $category = get_term_meta( $cat_ID, '_dahuzi_taxonomy_options', true );
    ...
    if( is_author() ){
        $html =  '作者_'.esc_html( get_the_author() ).'-'.get_bloginfo('name');
    }

    if( is_search() ){
        $html =  esc_html(get_search_query(), 1).'_搜索結果-'.get_bloginfo('name');
    }
