...
if ( ! class_exists( 'Dahuzi_delete_all_comments' ) ) {
	add_action( 'init', array( 'Dahuzi_delete_all_comments', 'get_object' ) );

	class Dahuzi_delete_all_comments {
		...
		public function remove_comments() {
			...
			if ( in_array( $pagenow, self::$comment_pages, false ) ) {
				wp_die(
					esc_html__( '此网站上的评论被禁用，如需开启，请到「后台-网站优化-WP优化」中开启', 'dahuzi_delete_all_comments' ),
					'',
					array( 'response' => 403 )
				);
				exit();
		...
		public function feed_links( $args ) {
			...
			$defaults = array(
				'separator' => _x(
					'&raquo;',
					'feed link',
					'dahuzi_delete_all_comments'
				),
				'feedtitle' => __( '%1$s %2$s Feed', 'dahuzi_delete_all_comments' ),
			);
		...
		public function xmlrpc_placeholder_method() {
			return new IXR_Error(
				403,
				esc_attr__( '此网站上的评论被禁用，如需开启，请到「后台-网站优化-WP优化」中开启', 'dahuzi_delete_all_comments' )
			);
		}
